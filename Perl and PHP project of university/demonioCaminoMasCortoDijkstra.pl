...
my $nombreFicheroEntrada;
my $nombreFicheroSalida;
...
print "tabla relaciones 0 0: ".$tablaRelaciones[0][0]."\n";#############################################
...
for($id_autor = 1; $id_autor <= ($id_autor_max); $id_autor++){
	...
	$tablaRelaciones[$id_autor][0] = 0;
	
	$ordenCoautor = 0;
		
	while($row = $query_handle->fetchrow_hashref()) {
			
		# if ($row->{'id_coautor'}<($id_autor_max / 1000)) {#############################################
		$tablaRelaciones[$id_autor][$ordenCoautor] = $row->{'id_coautor'};
	...
	while($row = $query_handle->fetchrow_hashref()) {
		...
		# if ($row->{'id_autor'}<($id_autor_max / 1000)) {#############################################			
		$tablaRelaciones[$id_autor][$ordenCoautor] = $row->{'id_autor'};
	...
	if ($id_autor==100000) {#############################################
		print "Vamos por el autor 100000\n\n"; #############################################
	...
	if ($id_autor==500000) {#############################################
		print "Vamos por el autor 500000\n\n";#############################################
		($sec,$min,$hour,$mday,$mon,$year,$wday,$yday,$isdst) = localtime(time);#############################################
		print  "########################################\n";#############################################
		printf "                        Hora de Fin: %02d:%02d:%02d %02d-%02d-%4d\n",$hour,$min,$sec,$mday,$mon+1,$year+1900;#############################################
		print  "########################################\n";#############################################
	...
	if ($id_autor==1000000) {#############################################
		print "Vamos por el autor 1000000\n\n";#############################################
		($sec,$min,$hour,$mday,$mon,$year,$wday,$yday,$isdst) = localtime(time);#############################################
		print  "########################################\n";#############################################
		printf "                        Hora de Fin: %02d:%02d:%02d %02d-%02d-%4d\n",$hour,$min,$sec,$mday,$mon+1,$year+1900;#############################################
		print  "########################################\n";#############################################
...
print "tabla relaciones 1 0: ".$tablaRelaciones[1][0]."\n";#############################################
print "tabla relaciones 1 1: ".$tablaRelaciones[1][1]."\n";#############################################
print "tabla relaciones 1 2: ".$tablaRelaciones[1][2]."\n";#############################################
print "tabla relaciones 1 3: ".$tablaRelaciones[1][3]."\n";#############################################
print "tabla relaciones 1 4: ".$tablaRelaciones[1][4]."\n";#############################################


print "tabla relaciones 3 0: ".$tablaRelaciones[3][0]."\n";#############################################
print "tabla relaciones 3 1: ".$tablaRelaciones[3][1]."\n";#############################################
print "tabla relaciones 3 2: ".$tablaRelaciones[3][2]."\n";#############################################
print "tabla relaciones 3 3: ".$tablaRelaciones[3][3]."\n";#############################################
print "tabla relaciones 3 4: ".$tablaRelaciones[3][4]."\n";#############################################


print "tabla relaciones 3661 0: ".$tablaRelaciones[3661][0]."\n";#############################################
print "tabla relaciones 3661 1: ".$tablaRelaciones[3661][1]."\n";#############################################
print "tabla relaciones 3661 2: ".$tablaRelaciones[3661][2]."\n";#############################################
print "tabla relaciones 3661 3: ".$tablaRelaciones[3661][3]."\n";#############################################
print "tabla relaciones 3661 4: ".$tablaRelaciones[3661][4]."\n";#############################################


print "tabla relaciones 600000 0: ".$tablaRelaciones[600000][0]."\n";#############################################
print "tabla relaciones 600000 1: ".$tablaRelaciones[600000][1]."\n";#############################################

print "tabla relaciones 1400000 0: ".$tablaRelaciones[1400000][0]."\n";#############################################
print "tabla relaciones 1400000 1: ".$tablaRelaciones[1400000][1]."\n";#############################################


# $nombreFicheroEntrada = "../tmp/autores.txt";
# $nombreFicheroSalida = "../tmp/camino.txt";
$nombreFicheroEntrada = "autores.txt";
$nombreFicheroSalida = "camino.txt";

while (1) {


	if ( -e $nombreFicheroEntrada ){
	
		open (FicheroEntrada,"<$nombreFicheroEntrada") || die "ERROR: No se pudo abrir el fichero $nombreFicheroEntrada\n";
	 
		$linea = <FicheroEntrada>;
	
		@arreglo = split("#",$linea);
	
		if (scalar(@arreglo)>1){
		
			$autorOrigen = $arreglo[0];
			$autorDestino = $arreglo[1];
			
			# truncate FicheroEntrada, 0;			
			...
			while (!$queue->empty && !$terminarBusqueda) {
				...
				$numCoautores = $#{$tablaRelaciones[$id_autor]}+1;
				if ($tablaRelaciones[$id_autor][0] == 0) { 
						$numCoautores = 0;
				}
				$ordenCoautor = 0;
				while($ordenCoautor<$numCoautores && !$terminarBusqueda) {

						$coautor = $tablaRelaciones[$id_autor][$ordenCoautor];
		...
		# open (FicheroEntrada,">$nombreFicheroEntrada") || die "ERROR: No se pudo abrir el fichero $nombreFicheroEntrada\n";
		# $linea2 = <FicheroEntrada>;linea2
		# if (linea2)
		# close (FicheroEntrada);	
		unlink($nombreFicheroEntrada);
	}
	
	sleep($demoraEntreChequeos);
